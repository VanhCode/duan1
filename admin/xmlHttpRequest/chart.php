...
function statistical($from,$to,$type='DATE'){
    ...
    ";
    $sql.="SELECT ".($type == 'MONTH' ? "DATE_FORMAT(dates.date, '%Y-%m')" : "$type(dates.date)")." AS date, COUNT(orders.order_id) AS num_orders, SUM(order_details.amount * order_details.price) AS revenue
    FROM dates
